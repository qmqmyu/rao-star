...
from rao.pddl.task import Task, Operator

# controls mass log output
verbose_logging = False

def ground(problem,remove_static=True):
    """
    ...
    static_pred_strs = _get_statics(predicates, actions)

    if remove_static:
        statics = static_pred_strs
        if verbose_logging:
    ...
    # Get the startic predicates in the initial state
    static_predicates = static_preds = [pred for pred in init if pred.split()[0].replace('(','') in static_pred_strs]
    ...
    name = problem.name
    return Task(name, facts, init, goals, operators, frozenset(static_predicates))
